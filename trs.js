...
const moviePosterUploadNotice = document.getElementById("moviePosterUploadNotice");
...
async function uploadAdminFile(file) {
  if (!(file instanceof File)) {
    throw new Error("Vui lòng chọn file hợp lệ");
  }
  ...
  if (!response.ok) {
    throw new Error(payload.message || payload.error || "Tải file thất bại");
  }
...
function bindPosterUploadButton({ fileInput, button, targetInput, noticeElement }) {
  if (!fileInput || !button || !targetInput || !noticeElement) {
    return;
  }
  ...
  button.addEventListener("click", async () => {
    ...
    if (!file) {
      noticeElement.textContent = "Vui lòng chọn file ảnh trước khi tải lên.";
      return;
    }

    try {
      button.disabled = true;
      noticeElement.textContent = "Đang tải ảnh lên...";
      const url = await uploadAdminFile(file);
      targetInput.value = url;
      noticeElement.textContent = "Tải ảnh thành công.";
    } catch (error) {
...
function setupMovieModalPosterUpload() {
  const posterUrlInput = document.getElementById("modal_posterUrl");
  if (!posterUrlInput) {
    return;
  }

  if (document.getElementById("modalPosterUploadSection")) {
    return;
  }

  const posterField = posterUrlInput.closest(".field");
  if (!posterField) {
    return;
  }

  const uploadSection = document.createElement("div");
  uploadSection.id = "modalPosterUploadSection";
  uploadSection.className = "field grid-span-all";
  uploadSection.innerHTML = `
    <label for="modalPosterFile">Chọn file poster từ máy</label>
    <input id="modalPosterFile" type="file" accept="image/*" title="Chọn file poster" />
    <div class="action-row mt-10">
      <button id="modalPosterUploadBtn" class="btn btn-light" type="button">Tải ảnh lên</button>
    </div>
    <p id="modalPosterUploadNotice" class="notice"></p>
  `;
  posterField.insertAdjacentElement("afterend", uploadSection);

  bindPosterUploadButton({
    fileInput: document.getElementById("modalPosterFile"),
    button: document.getElementById("modalPosterUploadBtn"),
    targetInput: posterUrlInput,
    noticeElement: document.getElementById("modalPosterUploadNotice"),
  });
}
...
bindPosterUploadButton({
  fileInput: moviePosterFileInput,
  button: moviePosterUploadBtn,
  targetInput: moviePosterUrlInput,
  noticeElement: moviePosterUploadNotice,
});
