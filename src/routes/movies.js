...
async function handle(req, res, pathname, context) {
	...
	if (pathname.match(/^\/api\/movies\/\d+\/reviews$/) && req.method === "GET") return await movieController.getMovieReviews(req, res, { ...context, pathname }), true;
	if (pathname.match(/^\/api\/movies\/\d+\/reviews$/) && req.method === "POST") return await movieController.createOrUpdateMovieReview(req, res, { ...context, pathname }), true;
